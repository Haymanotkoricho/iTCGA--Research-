#!/bin/bash

#SBATCH --job-name=autoAligner # you can give your job a name
#SBATCH --ntasks=5 # the number of processors or tasks
#SBATCH --account=itcga # our account
#SBATCH --reservation=2025_JUNE_ITCGA_WORKSHOP # this gives us special access during the workshop
#SBATCH --time=2:00:00 # the maximum time for the job hrs:min:secs
#SBATCH --mem=16gb # the amount of RAM
#SBATCH --partition=itcga # the specific server in chimera we are using
#SBATCH --error=%x-%A.err   # a filename to save error messages into
#SBATCH --output=%x-%A.out  # a filename to save any printed output into

# Load modules
module load gcc-10.2.0-gcc-9.3.0-f3oaqv7
module load python-3.8.12-gcc-10.2.0-oe4tgov
module load hisat2-2.1.0-gcc-9.3.0-u7zbyow
module load samtools-1.10-gcc-9.3.0-flukja5

# Write a for loop for each trimmed forward read file
for file in ~/1_project/data/trim_fastq/*R1_001_ds_trim.fastq;
do

# Take the basename of the selected file
base=$(basename $file _R1_001_ds_trim.fastq)

# Set variables
fwd=~/1_project/data/trim_fastq/${base}_R1_001_ds_trim.fastq
rev=~/1_project/data/trim_fastq/${base}_R2_001_ds_trim.fastq
ref=/itcgastorage/data01/itcga_workshops/aug2024_genomics/Genome/hg38/hg38
resdir=~/1_project/results

echo $base

# Run alignment
hisat2 -p 5 -x $ref -1 $fwd -2 $rev -S ${resdir}/sam/${base}.sam
echo "alignment done"

# Convert sam to bam
samtools view -S -b ${resdir}/sam/${base}.sam > ${resdir}/bam/${base}.bam
echo "conversion done"

# Sort bam file
samtools sort -o ${resdir}/bam/${base}.bam  ${resdir}/bam/${base}_sorted.bam
echo "sorting done"

# Calculate alignment stats
samtools flagstat ${resdir}/bam/${base}_sorted.bam
echo "stats done"

# Index alignment file
samtools index ${resdir}/bam/${base}_sorted.bam

done